...
# Every ship and scout scaffold also opens its Rules section with the canonical
# caveman activation: load and keep active the `caveman` skill at `full` from
# $HOME/.agents/skills/caveman/SKILL.md, so the compression posture is
# structural at launch rather than memory-dependent; compression stays
# chat-only, and secondmate charters do not carry the line (they inherit the
# captain-shared preference instead).
