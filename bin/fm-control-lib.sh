...
# refuses a --secondmate launch on them. A secondmate IS a firstmate, so it needs
# the emitted primary protocol in bin/fm-supervision-instructions.sh and a
# verified way to keep a watcher cycle alive; agy's worker mechanics being
# verified says nothing about either. The control plane
