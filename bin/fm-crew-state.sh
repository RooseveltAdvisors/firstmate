...
#      coarse runs-list attribution has no step detail to read, so a row whose
#      own status word is a live fix or CI phase never takes that override; the
#      emit site below owns that narrower rule.
