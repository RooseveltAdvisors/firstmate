...
Reach the real `tasks-axi` through `tests/lib.sh`'s `fm_test_tasks_axi`, never as a bare command: a bare call runs unbounded, so a wedged backlog backend hangs the whole run until CI kills it instead of failing the case.
