...
test_forced_child_missing_adapter_sibling_refuses_before_cleanup() {
  ...
  rc=0
  # The retirement-authority guard refuses a secondmate target before any
  # source check (its own contract is covered in tests/fm-secondmate-safety.test.sh),
  # so grant the exact authority and keep this assertion on the source refusal.
