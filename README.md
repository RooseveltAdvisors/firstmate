...
- **Optional secondmates** - opt in to persistent second mates that run from isolated firstmate homes with their own `FM_HOME`, state, projects, and session lock, supervising project clones or a project-less firstmate-repo domain.
  Local secondmate homes get guarded version sync and session-start recovery; the Stage 1 remote pilot adds transport to an already-provisioned Herdr home without remote lifecycle management.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca when explicitly selected); kill the session anytime and the next one reconciles local confirmed-dead secondmate agents and carries on.
...
| `/updatefirstmate` | Self-update the running firstmate and its local secondmates to the latest from origin with fast-forward-only pulls, then re-read instructions and nudge updated local secondmates |
