...
config/sandbox-workers-enabled  exact `v1` presence gate for the experimental Docker Sandboxes execution layer; LOCAL, gitignored, default-off, and not inherited; see docs/configuration.md "Docker Sandboxes worker layer"
config/sandbox-hosts.json  explicit role, transport, hostname, priority, resource-limit, capability-profile, auth-mode, and private-network-grant facts for sandbox hosts; LOCAL, gitignored, not inherited, and never an opaque scheduler score; see docs/configuration.md "Docker Sandboxes worker layer"
...
state/               volatile runtime signals; gitignored
  ...
  <id>.sandbox.json  machine-readable Docker Sandboxes ownership, host, stable sandbox id, work-copy, commit-sync, resource-limit, credential-presence, and lifecycle journal for an explicit `execution=sandbox` task; bin/fm-sandbox.sh owns exact fields and destructive proof
  <id>.sandbox-network.json  latest machine-readable sandbox-scoped sbx policy-log snapshot for an explicit sandbox task; bin/fm-sandbox.sh owns capture and teardown
...
An explicitly requested sandboxed worker remains a Herdr task and passes `--sandbox-host` in addition to `--backend herdr`; load no second runtime, and follow the default-off capability, credential, host-role, network, metadata, and cleanup contract in `docs/configuration.md` "Docker Sandboxes worker layer" and `bin/fm-sandbox.sh --help`.
