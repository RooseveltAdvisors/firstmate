...
config/gh-auth-skip  optional presence flag that skips the session-start GitHub auth check entirely, including the `gh auth status` invocation; LOCAL, gitignored, and not inherited; see docs/configuration.md "Toolchain"
...
Do not dispatch until the required tools are present and, unless the home carries `config/gh-auth-skip`, GitHub authentication is good.
