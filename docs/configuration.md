...
agy typed-submit confirmation is verified on tmux only, for the same structural reason: its composer is a bare `>` between rules rather than a bordered container, so the classifier's dead-shell-prompt safety rule makes every agy composer verdict `unknown`, and only tmux's submit core resolves that through the busy footer; the brief itself rides the launch command rather than `fm-send`, so a spawn is unaffected. [runtime backend verification](verification/runtime-backends.md#typed-submit-confirmation-is-a-tmux-only-boundary) owns the evidence.
