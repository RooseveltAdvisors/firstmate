...
claude, codex, opencode, pi, pi-signed, grok, kimi, and cursor are empirically verified for crewmate and secondmate launches; muse and agy are verified for crewmate and scout launches only and `fm-spawn` refuses a secondmate on either, because neither has a primary supervision protocol; [README requirements](../README.md#requirements) own the set supported for the primary session.
...
The spawn records the paths it registered in `state/<id>.agy-trust`, and `fm-teardown.sh` withdraws exactly those with `fm-agy-trust.sh --remove`, so a settings file firstmate does not own does not accumulate one dead worktree path per task.
Withdrawing only what that record names is what keeps a workspace the operator trusted by hand out of it: a task with no record touches nothing, whatever path it ran in.
