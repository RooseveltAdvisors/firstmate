...
A held row is never a candidate: the scan itself excludes held in_progress rows before endpoint classification, so check and dry-run output cannot present a held row as reclaimable.
