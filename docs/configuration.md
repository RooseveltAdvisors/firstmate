...
FM_SECONDMATE_IDLE_STALE_SECS=2700 # seconds an evergreen secondmate's pane may sit unchanged before the normal-mode watcher surfaces it as a stopped standing loop; a ping-model secondmate stays exempt however long it idles, and away mode leaves a non-paused secondmate alone as before
FM_SECONDMATE_HOME_SCAN_TTL_SECS=300 # seconds a memoized "a crew in this home is working" verdict may stand for an otherwise unchanged home; a new pane hash or a status append in that home bypasses it at once
