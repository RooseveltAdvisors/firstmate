...
`fm-teardown.sh` withdraws that entry again with `fm-agy-trust.sh --remove`, so a settings file firstmate does not own does not accumulate one dead worktree path per task; the withdrawal writes nothing when the path was never registered.
