...
Automatic transitions run from the configured data directory's parent, letting that home's effective tasks-axi configuration address its selected adapter while keeping relocated backlog configuration, archives, and relative scout-report links together.
A legacy home without `.tasks.toml` receives an explicit `<data>/backlog.md` fallback on mutations, preserving relocated markdown backlogs without overriding a configured adapter.
...
Anything merely unproven (an unreadable pane, an unreachable backend or remote) is kept.
A row carrying the `tasks-axi-held` label is classified `held`, kept without a liveness probe, and never counted as reclaimable; the reclaim's re-proof still refuses a row that becomes held mid-apply.
