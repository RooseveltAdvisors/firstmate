...
On a beads home, a successful dispatch commit also stamps the spawned worker as its backlog bead's assignee with `bd assign`, resolved from the same root `.tasks.toml` `[beads]` section; the stamp is best-effort, so it never fails the spawn, a markdown home and a secondmate spawn's expected missing bead skip quietly, and a crewmate or scout spawn warns on stderr when the assign attempt itself fails (a failed read stays silent); the stamp lands only on a reliably read unassigned bead, so an existing assignee is preserved as ownership evidence and an unreadable or ambiguous read leaves assignment unchanged rather than guessing; on a bd that supports conditional updates (`bd update --if-assignee`) the stamp is one atomic guarded write, so an owner recorded between the read and the stamp wins and the lost race defers quietly, while an older bd falls back to the unconditional `bd assign` shorthand; an interruption deferred across the landed dispatch commit attempts the same stamp on its exit path before reporting the preserved state; a relaunch is not a dispatch and never re-stamps, so a bead reassigned since the task's first spawn keeps its current owner ([`bin/fm-backlog-transition-lib.sh`](../bin/fm-backlog-transition-lib.sh)'s `fm_beads_assignee` and `fm_beads_assign`; [`tests/fm-spawn-beads-assign.test.sh`](../tests/fm-spawn-beads-assign.test.sh)).
