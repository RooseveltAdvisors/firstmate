...
| `fm-ssh-lib.sh`          | Strict parent-initiated SSH argv transport for existing commands in remote Secondmate homes |
...
| `fm-config-push.sh`      | Push declared inherited local material to local or remote live secondmates and send a pointer to the literal-content config reread when config changed |
