...
A PR-requiring ship `done:` is captain-relevant only after the branch is on origin and an open PR is referenced; otherwise the watcher drops that completion and steers the worker to push (`bin/fm-done-guard-lib.sh`).
