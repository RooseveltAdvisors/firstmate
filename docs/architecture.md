...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, evergreen secondmates the normal-mode watcher finds idle past `FM_SECONDMATE_IDLE_STALE_SECS` with nothing running in their own homes, and heartbeat backstop hits.
...
A secondmate's idle endpoint is judged by the posture recorded in its `data/secondmates.md` route line instead: a `ping-model` secondmate keeps the original exemption because it is built to wait for externally timed pings, while an evergreen secondmate owns a standing loop, so the normal-mode watcher surfaces one stale wake through the ordinary non-terminal path for a pane unchanged past `FM_SECONDMATE_IDLE_STALE_SECS` with nothing provably working in its own home, and away mode is unchanged by that rule because the watcher leaves a non-paused secondmate alone there exactly as before.
`bin/fm-classify-lib.sh` owns that posture contract, its evergreen default for an absent or unrecognized value, and the threshold constant.
...
Whether an idle secondmate pane is healthy follows the posture recorded in its route line, owned by `bin/fm-classify-lib.sh`; teardown is explicit and refuses while the secondmate home has in-flight work unless the captain has approved discard with `--force`.
