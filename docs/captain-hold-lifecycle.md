...
Only when no row carries that marker line is the legacy id tried under the configured beads prefix, and that name-only guess is accepted solely for a single candidate row - one still held for the captain, or one whose body carries a recorded captain answer, so an answered-and-released call still resolves for a later complete/verify; two candidate rows refuse rather than attest.
...
The markdown-to-beads migration family runs the same suite's beads fixture (bd-driven scratch graph, self-skipping on markdown-only tasks-axi installs) and proves: `verify` and `complete` resolve an attested legacy id through a migrated row's marker note, through the configured prefix when no row carries a note - naming the resolved row in the completion line - and through the marker note of a pre-collapse derived identity; a released prefix-resolved call still resolves through the prefix fallback after its `--release` answer; a marker-noted row wins over an unrelated candidate row occupying the bare prefix namesake; an unresolvable id is refused once naming the id (never an empty name); and the attested id stays in `decision_keys=` for idempotent re-verification.
