...
On the Beads backend, an attested legacy markdown id that resolves to no task is accepted through the row the markdown-to-beads hold migration produced: the legacy id under the configured beads prefix, or a row whose notes carry the exact marker line `migrated from data/backlog.md id <legacy id>` (what fm-hold-migration wrote on 2026-09-04).
...
The markdown-to-beads migration family runs the same suite's beads fixture (bd-driven scratch graph, self-skipping on markdown-only tasks-axi installs) and proves: `verify` and `complete` resolve an attested legacy id through a migrated row's marker note, through the configured prefix when no note exists, refuse an unresolvable id once naming the id (never an empty name), and keep the attested id in `decision_keys=` for idempotent re-verification.
