...
- A valid secondmate home is in scope; an idle secondmate endpoint with no X-mode relay poll is left to the watcher's posture test in `bin/fm-classify-lib.sh` rather than judged here.
