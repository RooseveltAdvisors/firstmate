...
- `NEEDS_GH_AUTH` - ask the captain to run `! gh auth login` (interactive; you cannot run it for them).
  Where `gh auth status` itself is unusable because it blocks on a credential-store read or raises a per-invocation approval prompt, the captain can instead opt that home out with `config/gh-auth-skip` (`docs/configuration.md` "Toolchain" owns the flag), after which the check never runs and its GitHub auth is unchecked rather than proven good.
